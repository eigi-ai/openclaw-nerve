...
function loadWidth(): number {
  try {
    ...
    return v
      ? Math.max(MIN_WIDTH, Math.min(MAX_WIDTH, Number(v)))
      : DEFAULT_WIDTH;
  } catch {
...
export function FileTreePanel({
  ...
  const showToast = useCallback(
    (nextToast: FileTreeToast, timeoutMs?: number) => {
      clearToastTimer();
      setToast(nextToast);
  ...
  useEffect(() => {
    ...
    const maxX = Math.max(
      ...
      window.innerWidth - width - MENU_VIEWPORT_PADDING,
    );
    const maxY = Math.max(
      MENU_VIEWPORT_PADDING,
      window.innerHeight - height - MENU_VIEWPORT_PADDING,
    );

    const nextX = Math.min(
      Math.max(contextMenu.x, MENU_VIEWPORT_PADDING),
      maxX,
    );
    const nextY = Math.min(
      Math.max(contextMenu.y, MENU_VIEWPORT_PADDING),
      maxY,
    );
  ...
  const toggleCollapsed = useCallback(() => {
    ...
    try {
      localStorage.setItem(COLLAPSED_STORAGE_KEY, String(collapsedRef.current));
    } catch {
      /* ignore */
    }
  }, []);
    ...
    async (sourcePath: string, targetDirPath: string) => {
      try {
        ...
        if (targetDirPath === ".trash" && !sourcePath.startsWith(".trash/")) {
          ...
          const result = await postFileOp<FileOpResult>("/api/files/trash", {
            path: sourcePath,
          });
        ...
        selectFile(result.to);
        showToast(
          { type: "success", message: `Moved ${basename(result.from)}` },
          3000,
        );
      } catch (err) {
  ...
  const canDropToTarget = useCallback(
    (source: TreeEntry, targetDirPath: string): boolean => {
  ...
  const startRename = useCallback(
    (entry: TreeEntry) => {
      if (entry.path === ".trash") {
        showToast(
          { type: "error", message: "Cannot rename .trash root" },
          3500,
        );
        return;
      }
  ...
  const commitRename = useCallback(async () => {
    ...
    try {
      ...
      selectFile(result.to);
      showToast(
        { type: "success", message: `Renamed to ${basename(result.to)}` },
        3000,
      );
    } catch (err) {
  ...
  const moveToTrash = useCallback(
    async (entry: TreeEntry) => {
      ...
      try {
        ...
        const message =
          err instanceof Error ? err.message : "Move to Trash failed";
        showToast({ type: "error", message }, 4500);
        setContextMenu(null);
      }
    },
    ...
    async (entry: TreeEntry) => {
      try {
        ...
        showToast(
          {
            type: "success",
            message: `Permanently deleted ${basename(result.from)}`,
          },
          3000,
        );
      } catch (err) {
        const message =
          err instanceof Error ? err.message : "Permanent deletion failed";
        showToast({ type: "error", message }, 4500);
      } finally {
  ...
  const restoreEntry = useCallback(
    async (entryPath: string) => {
      try {
        const result = await postFileOp<FileOpResult>("/api/files/restore", {
          path: entryPath,
        });
        refresh();
        onRemapOpenPaths?.(result.from, result.to);
        selectFile(result.to);
        showToast(
          { type: "success", message: `Restored ${basename(result.to)}` },
          3000,
        );
      } catch (err) {
  ...
  const handleDragStart = useCallback(
    (entry: TreeEntry, event: React.DragEvent) => {
  ...
  const handleDragOverDirectory = useCallback(
    (entry: TreeEntry, event: React.DragEvent) => {
  ...
  const handleDragLeaveDirectory = useCallback(
    (entry: TreeEntry, event: React.DragEvent) => {
  ...
  const handleDropDirectory = useCallback(
    (entry: TreeEntry, event: React.DragEvent) => {
      event.preventDefault();
  ...
  const showTrashAction = Boolean(
    menuEntry && !menuPath.startsWith(".trash") && menuPath !== ".trash",
  );
    ...
    >
      ...
      {contextMenu && menuEntry && (
        ...
        >
          ...
          {showTrashAction && (
            ...
            >
              <Trash2 size={12} />
              {workspaceInfo?.isCustomWorkspace
                ? "Permanently Delete"
                : "Move to Trash"}
            </button>
          )}
      ...
      {toast && (
        <div className="fixed bottom-4 left-4 z-[70] w-fit min-w-[320px] max-w-[min(92vw,680px)] px-4 py-2.5 rounded-md border text-xs shadow-lg bg-card flex items-center gap-3">
          <span
            className={`flex-1 ${toast.type === "error" ? "text-destructive" : "text-foreground"}`}
          >
            {toast.message}
          </span>
          {toast.type === "undo" && (
