  ...
  function ChatPanel(
    ...
    useEffect(() => {
      if (search.currentMatch) {
        const msgElement = messageRefs.current.get(
          search.currentMatch.messageIndex,
        );
        if (msgElement && scrollRef.current) {
          ...
          const searchCollapseKey =
            searchMsg?.msgId || searchMsg?.tempId || msgIndex;
    ...
    useEffect(() => {
      if (!autoScroll && messages.length > prevMessageCount.current) {
        setUnreadCount(
          (prev) => prev + (messages.length - prevMessageCount.current),
        );
      }
    ...
    return (
      <div
        id={id}
        className="h-full flex flex-col border-r border-border min-w-0 relative"
      >
        {/* COMMS Header */}
        <ChatHeader
          onReset={onReset}
        ...
        >
          ...
          {hasMore && (
            <div
              ref={sentinelRef}
              className="flex items-center justify-center py-2 text-muted-foreground/60 text-[10px] tracking-widest uppercase select-none"
            >
              ↑ older messages
            </div>
          )}
          {messages.map((msg, i) => {
            ...
            const isCollapsed =
              ...
              (msg.isThinking || isMessageCollapsible(msg));
            ...
            const stableKey =
              msg.msgId ||
              msg.tempId ||
              `${msg.role}-${msg.timestamp.getTime()}-${i}`;
