...
export const config = {
  ...
  whisperModelDir:
    process.env.WHISPER_MODEL_DIR || path.join(HOME, ".nerve", "models"),
  ...
  edgeVoiceGender:
    process.env.EDGE_VOICE_GENDER === "male" ||
    process.env.EDGE_VOICE_GENDER === "female"
  ...
  usageFile:
    process.env.USAGE_FILE || path.join(HOME, ".openclaw", "token-usage.json"),
  ...
  sessionTtlMs: Number(
    process.env.NERVE_SESSION_TTL || 30 * 24 * 60 * 60 * 1000,
  ), // 30 days

  // Remote proxy mode — proxy file/memory/session/token operations to vaani_core's
  // cockpit-proxy instead of reading the local filesystem. Used when Nerve is
  // shared (nerve.eigi.ai). Credentials (API key + URL) come per-request from
  // the frontend via X-Proxy-Api-Key and X-Proxy-Api-Url headers.
  remoteMode:
    (process.env.NERVE_REMOTE_MODE || "false").toLowerCase() === "true",
} as const;
  ...
  switch (key) {
    case "sttProvider":
      if (value !== "local" && value !== "openai") {
        throw new Error(
          `Invalid sttProvider: ${value}. Must be 'local' or 'openai'`,
        );
      }
      break;
    case "language":
      if (typeof value !== "string" || !SUPPORTED_LANGUAGE_CODES.has(value)) {
        throw new Error(
          `Invalid language: ${value}. Must be a supported ISO 639-1 code`,
        );
      }
      break;
    case "edgeVoiceGender":
      if (value !== "female" && value !== "male") {
        throw new Error(
          `Invalid edgeVoiceGender: ${value}. Must be 'female' or 'male'`,
        );
      }
      break;
    case "sessionSecret":
...
export function printStartupBanner(version: string): void {
  ...
  console.log(
    `  Agent: ${config.agentName} | TTS: ${ttsProviderLabel()} | STT: ${sttProviderLabel()}`,
  );
  console.log(`  Gateway: ${config.gatewayUrl}`);
  if (config.remoteMode) {
    console.log(
      `  \x1b[36m🔗 Remote proxy mode → credentials per-request via headers\x1b[0m`,
    );
  }
  if (config.auth) {
...
export function validateConfig(): void {
  // ── Remote proxy mode validation ─────────────────────────────────
  if (config.remoteMode) {
    console.log(
      "  \x1b[36m🔗 Remote proxy mode enabled — file/memory/session/token ops will be proxied via vaani_core\x1b[0m",
    );
  }
  ...
  if (config.auth && !config.sessionSecret) {
    ...
    console.warn(
      "[config] ⚠ NERVE_SESSION_SECRET not set — generated ephemeral secret (sessions won't survive restarts)",
    );
  ...
  if (!config.openaiApiKey) {
    console.warn(
      "[config] ⚠ OPENAI_API_KEY not set — OpenAI TTS/Whisper unavailable (Edge TTS still works)",
    );
  }
  if (!config.replicateApiToken) {
    console.warn(
      "[config] ⚠ REPLICATE_API_TOKEN not set — Qwen TTS unavailable",
    );
  }
  if (!process.env.NERVE_LANGUAGE && process.env.LANGUAGE) {
    console.warn(
      "[config] ⚠ LANGUAGE is deprecated — use NERVE_LANGUAGE instead",
    );
  }
  if (config.host === "0.0.0.0" && config.auth) {
    console.warn(
      "[config] ⚠ Server binds to 0.0.0.0 — API is accessible from the network (auth enabled).",
    );
  } else if (
    config.host !== "0.0.0.0" &&
    ...
    console.warn(
      "[config] ⚠ Server binds to " +
        config.host +
        " — API may be accessible from the network.\n" +
  ...
  if (config.fileBrowserRoot) {
    try {
      ...
      console.warn(
        "[config] ⚠ FILE_BROWSER_ROOT is not a directory or is inaccessible, falling back to default",
      );
