...
/** Check if we're in remote proxy mode (apiKey present in oc-config). */
...
export function useDashboardData(
  ...
  const refreshMemoriesRef = useRef<
  ...
  const refreshMemories = useCallback(async (signal?: AbortSignal) => {
    try {
      ...
      if (err instanceof Error && err.name !== "AbortError") {
        console.debug(
          "[DashboardData] Failed to refresh memories:",
          err.message,
        );
      }
    } finally {
  ...
  useEffect(() => {
    ...
    return subscribe((msg: GatewayEvent) => {
      ...
      if (evt === "chat" && payload?.state === "final") {
        ...
        // In proxy mode, no local file watcher exists so SSE file.changed never fires.
        // After an agent response completes it may have written files — trigger a tree
        // refresh through the same onFileChanged pipeline that SSE file.changed uses.
        // A unique path avoids the dedup guard in FileTreePanel; paths without '/'
        // resolve to parentDir '' which refreshes the root tree.
        if (isProxyMode()) {
          setTimeout(
            () => onFileChangedRef.current?.(`__chat_done_${Date.now()}`),
            1000,
          );
        }
      }
  ...
  useEffect(() => {
    ...
    // Safety-net polling — gateway WS provides real-time updates in both
    // local and proxy modes, so these long intervals are just fallbacks.
    const memIv = setInterval(
      () => refreshMemories(controller.signal),
      MEMORY_POLL_INTERVAL,
    );
    const tokIv = setInterval(
      () => refreshTokens(controller.signal),
      TOKEN_POLL_INTERVAL,
    );
